...
const cppBackendConfig = {
  ...
  host: "localhost",
  port: 5001, // Change to your C++ server port
...
function updateOverlayThumb() {
  ...
  const thumbTop = Math.max(
    0,
    Math.min(maxThumbTop, scrollRatio * maxThumbTop)
  );
...
function positionOverlay() {
  ...
  const left = Math.round(
    rect.right - overlayScrollbar.offsetWidth - offsetRight
  );
...
if (chatBox)
  overlayPositionObserver.observe(chatBox, { childList: true, subtree: true });
...
document.addEventListener("mousemove", (e) => {
  ...
  const newThumbTop = Math.max(
    0,
    Math.min(maxThumbTop, startScrollTop / pixelsPerScroll + deltaY)
  );
...
if (overlayScrollbar) {
  overlayScrollbar.addEventListener("click", (e) => {
    ...
    const ratio = Math.max(
      0,
      Math.min(1, (clickY - thumbHeight / 2) / maxThumbTop)
    );
    const scrollable = contentHeight - visibleHeight;
...
function applyScrollbarClass() {
  ...
  if (currentScrollbarEl)
    currentScrollbarEl.textContent =
      currentScrollbar.charAt(0).toUpperCase() + currentScrollbar.slice(1);
}
...
async function getCppBackendResponse(modelName, userMessage) {
  ...
  try {
    ...
    return (
      data.reply || data.response || "Sorry, I could not process your message."
    );
  } catch (error) {
...
document.addEventListener("click", (e) => {
  ...
  if (!scrollbarDropdown.classList.contains("hidden")) {
    if (
      !scrollbarDropdown.contains(e.target) &&
      e.target !== changeScrollbarBtn
    ) {
      toggleScrollbarDropdown(false);
    }
  }
...
window.addEventListener("resize", () => {
  if (!scrollbarDropdown || scrollbarDropdown.classList.contains("hidden"))
    return;
  positionScrollbarDropdown();
});
